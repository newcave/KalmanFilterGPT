...
#   st.beta_set_page_config(page_title="Kalman Filter", page_icon=":bar_chart:", layout="wide", initial_sidebar_state="expanded")
    ...
    sidebar = st.sidebar
    time_end = sidebar.slider("Time end (hrs.) [총 분석할 시간]", min_value=1, max_value=48, value=12, step=1)
    dt = sidebar.slider("Time step (hrs.) [분석할 간격, 빈도]", min_value=0.1, max_value=1.0, value=0.4, step=0.1)
